...
// parseNonStopListLine parses a directory line in the format used by HP NonStop (Tandem)
// EMPLYE          0            12288 30-Nov-25 10:47:56 110, 99 "nnnn"
func parseNonStopListLine(line string, now time.Time, loc *time.Location) (*Entry, error) {
	scanner := newScanner(line)
	fields := scanner.NextFields(6)
	if len(fields) < 6 {
		return nil, errUnsupportedListLine
	...
	// Field 1: Code (number)
	// Field 2: Size (number)
	// Field 3: Date (DD-Mon-YY)
	// Field 4: Time (HH:MM:SS)

	// Check if Field 1 and 2 are numbers
	...
	// Check date format: 30-Nov-25 or 5-Dec-25
	// The day part can be 1 or 2 digits
	dateParts := strings.Split(fields[3], "-")
	if len(dateParts) != 3 {
		return nil, errUnsupportedListLine
	}
	...
	// Parse time: 30-Nov-25 10:47:56 or 5-Dec-25 11:46:36
	timeStr := fields[3] + " " + fields[4]
	// Go layout: 02-Jan-06 15:04:05
	// Note: Go's time parser is flexible with single digit days if we use _2
	t, err := time.ParseInLocation("_2-Jan-06 15:04:05", timeStr, loc)
	if err != nil {
		return nil, err
	}
	e.Time = t
