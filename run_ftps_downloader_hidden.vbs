...
End If
' Format: ftps-downloader-restart-YYYY-MM-DD.log
logDate = Year(Now) & "-" & Right("0" & Month(Now), 2) & "-" & Right("0" & Day(Now), 2)
logFile = logsDir & "\ftps-downloader-restart-" & logDate & ".log"
...
' Check if ftps-downloader.exe is already running, terminate if so
Set objWMI = GetObject("winmgmts:\\.\root\cimv2")
Set colProcesses = objWMI.ExecQuery("SELECT * FROM Win32_Process WHERE Name = 'ftps-downloader.exe'")
processCount = 0
For Each objProcess In colProcesses
    processCount = processCount + 1
    WriteLog "Terminating existing ftps-downloader.exe (PID: " & objProcess.ProcessId & ")"
