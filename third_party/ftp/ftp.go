...
// RawList issues a LIST FTP command and returns raw server lines without parsing.
// Useful for non-standard directory formats (e.g. NonStop/Guardian) that the
// default parser cannot understand.
func (c *ServerConn) RawList(path string) (lines []string, err error) {
	cmd := "LIST"
